...
基于用户的协同过滤推荐算法
-------------
对于一个已经读过几本书的用户，我们采用的推荐方法便是采用正常的基于用户的协同过滤算法。基于用户的协同过滤推荐算法的基本思想便是：首先依据依据用户对物品的评价计算出所有用户之间的相似度，之后选出与当前用户最相似的N个用户，再用N个邻居用户对物品的评分，预测当前用户对没有浏览过的物品的可能评分，最后按照预测出的可能评分的高低向当前用户推荐物品。 接下来将对算法的实现进行详细介绍。</br>算法在程序中的位置为recommendsystem1.cpp中的```c++User:: getrecommendation()```中。首先我们要先进行对与所有用户的相似度计算。计算的基本公式如下：
